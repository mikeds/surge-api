 ...
 */
class Pastor_Controller extends Api_Controller {
	protected
		$_account = null;
	...
	public function get_balance() {
		...
		$row = $this->accounts->get_datum(
			'',
			array(
				'pastor_accounts.oauth_bridge_id' => $oauth_bridge_id
			),
			array(),
			array(
				array(
					'table_name' 	=> 'wallet_addresses',
					'condition'		=> 'wallet_addresses.oauth_bridge_id = pastor_accounts.oauth_bridge_id'
				)
			)
		)->row();
	...
	private function validate_access() {
		...
		$inner_joints = array(
			array(
				...
				'condition'		=> 'pastor_accounts.oauth_bridge_id = oauth_bridges.oauth_bridge_id'
			),
			array(
				'table_name' 	=> 'wallet_addresses',
				'condition'		=> 'wallet_addresses.oauth_bridge_id = pastor_accounts.oauth_bridge_id'
		...
		$row = $this->bridges->get_datum(
			'',
			array(
				'pastor_accounts.oauth_bridge_id' => $client_id
			),
			array(),
			$inner_joints,
			array(
				'*',
				'pastor_accounts.oauth_bridge_id as account_oauth_bridge_id'
			)
		)->row();
